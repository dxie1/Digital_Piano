// Original File Name: ADCSWTrigger.c
// Modified File Name: ADC.c
...
// ===========Di and Jesus: This file deals with ADC drivers ========================
...
// SS3 1st sample source: programmable using variable 'channelNum' [0:11]
// SS3 interrupts: enabled but not promoted to controller
void ADC1_InitSWTriggerSeq3(unsigned char channelNum){ volatile unsigned long delay;
  switch(channelNum){             // 1) activate clock
    case 0:
    case 1:
    case 2:
...
//      break;
  ...
  SYSCTL_RCGC0_R |= 0x00020000;   // 7) activate ADC1 (legacy code)
  SYSCTL_RCGCADC_R |= 0x00000002; // 7) activate ADC1 (actually doesn't work)
  delay = SYSCTL_RCGC0_R;         // 8) allow time for clock to stabilize
  delay = SYSCTL_RCGC0_R;
//  SYSCTL_RCGC0_R &= ~0x00000300;  // 9) configure for 125K (legacy code)
...
// Input: none
// Output: 12-bit result of ADC conversion
unsigned long ADC1_InSeq3(void){  unsigned long result;
